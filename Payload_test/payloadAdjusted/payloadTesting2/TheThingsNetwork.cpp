...
TheThingsNetwork::TheThingsNetwork(Stream &modemStream, Stream &debugStream, ttn_fp_t fp, uint8_t sf) // Removed uint8_t fsb because unneccessary
{
...
bool TheThingsNetwork::personalize(const char *devAddr, const char *nwkSKey, const char *appSKey) //  bool resetFirst removed
{
  // if(resetFirst) {
  //   reset(adr);
  // }
...
{
  //configureChannels(fsb); Only EU868 neccessary
  configureEU868();
  setSF(sf);
...
bool TheThingsNetwork::provision(const char *appEui, const char *appKey) // bool resetFirst removed
{
  // if(resetFirst) {
  //   reset(adr);
  // }
...
bool TheThingsNetwork::join(const char *appEui, const char *appKey, int8_t retries, uint32_t retryDelay) // Removed lorawan_class_t p_lw_class
{
...
ttn_response_t TheThingsNetwork::poll(port_t port, bool confirm) // Removed bool modem_only
{
  switch(lw_class)
  {
...
//} Weg omdat dit niet voor de EU zone is 
...
//     configureIN865_867();
//   break;*/ // Weg omdat dit niet voor de EU zone is
// default:
...
{
  uint8_t dr = 12 - sf; // afgeleid uit de default die ook wordt aangeroepen wanneer EU de zone is
  //switch (fp)
  //{
  //case TTN_FP_EU868:
  //case TTN_FP_IN865_867:
