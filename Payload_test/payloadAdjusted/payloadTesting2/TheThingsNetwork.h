...
private:
  ...
  void autoBaud();
  void configureEU868();
  //void configureUS915(uint8_t fsb); Weg omdat dit niet voor de EU zone is 
  //void configureAU915(uint8_t fsb); Weg omdat dit niet voor de EU zone is 
  //void configureAS920_923(); Weg omdat dit niet voor de EU zone is 
  //void configureAS923_925(); Weg omdat dit niet voor de EU zone is 
  //void configureKR920_923(); Weg omdat dit niet voor de EU zone is 
  //void configureIN865_867(); Weg omdat dit niet voor de EU zone is 
  //void configureChannels(uint8_t fsb); Weg omdat deze niet wordt gebruikt voor EU
...
public:
  ...
  TheThingsNetwork(Stream &modemStream, Stream &debugStream, ttn_fp_t fp, uint8_t sf = TTN_DEFAULT_SF); /// Removed: uint8_t fsb = TTN_DEFAULT_FSB); because fsb not neccessary for EU868
  ...
  bool join(const char *appEui, const char *appKey, int8_t retries = -1, uint32_t retryDelay = 10000); //(, lorawan_class_t = CLASS_A) removed
  bool join(int8_t retries = -1, uint32_t retryDelay = 10000); // (const char *devEui, const char *appEui, const char *appKey, ) removed
